<template>
  <div style="margin-top: 60px; margin-left: 80px; border: 0px solid red">
    ...
    >
      ...
      <el-table-column prop="buildingName" label="楼宇" width="160">
      </el-table-column>
      <el-table-column prop="name" label="名称" width="160"> </el-table-column>
      <el-table-column prop="type" label="几人间" width="160">
      </el-table-column>
      <el-table-column prop="available" label="空余床位" width="160">
      </el-table-column>
...
export default {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios
            .get("http://localhost:9000/dormitory/search", {
    ...
    page(currentPage) {
      ...
      if (_this.ruleForm.value == "") {
        axios
          .get(
            "http://localhost:9000/dormitory/list/" +
              currentPage +
              "/" +
              _this.pageSize
          )
    ...
    del(row) {
      ...
      this.$confirm("确认删除【" + row.name + "】吗？", "提示", {
        ...
        axios
          .delete("http://localhost:9000/dormitory/deleteById/" + row.id)
          .then(function (resp) {
            if (resp.data.code == 0) {
  ...
  created() {
    const _this = this;
    axios
      .get("http://localhost:9000/dormitory/list/1/" + _this.pageSize)
      .then(function (resp) {
        _this.tableData = resp.data.data.data;
        _this.total = resp.data.data.total;
      });
