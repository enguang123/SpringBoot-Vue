...
export default {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios
            .get("http://localhost:9000/building/search", {
    ...
    page(currentPage) {
      ...
      if (_this.ruleForm.value == "") {
        axios
          .get(
            "http://localhost:9000/building/list/" +
              currentPage +
              "/" +
              _this.pageSize
          )
    ...
    del(row) {
      ...
      this.$confirm("确认删除【" + row.name + "】吗？", "提示", {
        ...
        axios
          .delete("http://localhost:9000/building/deleteById/" + row.id)
          .then(function (resp) {
            if (resp.data.code == 0) {
  ...
  created() {
    const _this = this;
    axios
      .get("http://localhost:9000/building/list/1/" + _this.pageSize)
      .then(function (resp) {
        _this.tableData = resp.data.data.data;
        _this.total = resp.data.data.total;
      });
