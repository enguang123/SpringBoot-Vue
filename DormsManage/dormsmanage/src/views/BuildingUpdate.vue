    ...
    >
      ...
      <el-form-item label="管理员" prop="adminId">
        <el-select v-model="ruleForm.adminId" placeholder="请选择管理员">
          <el-option
            v-for="item in dormitoryAdminList"
            :label="item.name"
            :value="item.id"
          ></el-option>
        </el-select>
...
export default {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios
            .put("http://localhost:9000/building/update", _this.ruleForm)
            .then(function (resp) {
              if (resp.data.code == 0) {
  ...
  created() {
    const _this = this;
    axios
      .get("http://localhost:9000/dormitoryAdmin/list")
      .then(function (resp) {
        _this.dormitoryAdminList = resp.data.data;
      });
    axios
      .get("http://localhost:9000/building/findById/" + _this.$route.query.id)
      .then(function (resp) {
