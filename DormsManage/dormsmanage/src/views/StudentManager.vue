<template>
  <div style="margin-top: 60px; margin-left: 80px; border: 0px solid red">
    ...
    >
      ...
      <el-table-column prop="createDate" label="入住时间" width="130">
      </el-table-column>
...
export default {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios
            .get("http://localhost:9000/student/search", {
    ...
    del(row) {
      ...
      this.$confirm("确认删除【" + row.name + "】吗？", "提示", {
        ...
        axios
          .delete("http://localhost:9000/student/deleteById/" + row.id)
          .then(function (resp) {
            if (resp.data.code == 0) {
  ...
  created() {
    const _this = this;
    axios
      .get("http://localhost:9000/student/list/1/" + _this.pageSize)
      .then(function (resp) {
        _this.tableData = resp.data.data.data;
        _this.total = resp.data.data.total;
      });
