...
export default {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios
            .put("http://localhost:9000/student/update", _this.ruleForm)
            .then(function (resp) {
              if (resp.data.code == 0) {
  ...
  created() {
    const _this = this;
    axios
      .get("http://localhost:9000/dormitory/availableList")
      .then(function (resp) {
        _this.dormitoryList = resp.data.data;
        // console.log(resp.data.data);
      });
    axios
      .get("http://localhost:9000/student/findById/" + _this.$route.query.id)
      .then(function (resp) {
        // console.log(resp.data.data);
