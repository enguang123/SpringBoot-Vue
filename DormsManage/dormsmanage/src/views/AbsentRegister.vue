...
export default {
  ...
  methods: {
    buildingChange() {
      ...
      axios
        .get(
          "http://localhost:9000/absent/findDormitoryByBuildingId/" +
            _this.ruleForm.buildingId
        )
        .then(function (resp) {
    ...
    dormitoryChange() {
      ...
      axios
        .get(
          "http://localhost:9000/absent/findStudentByDormitoryId/" +
            _this.ruleForm.dormitoryId
        )
        .then(function (resp) {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios
            .post("http://localhost:9000/absent/save", _this.ruleForm)
            .then(function (resp) {
              if (resp.data.code == 0) {
                _this.$alert("缺寝登记成功", "", {
  ...
  created() {
    const _this = this;
    axios
      .get("http://localhost:9000/absent/buildingList")
      .then(function (resp) {
