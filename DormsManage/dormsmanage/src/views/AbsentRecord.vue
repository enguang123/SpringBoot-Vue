<template>
  <div style="margin-top: 60px; margin-left: 80px; border: 0px solid red">
    ...
    >
      ...
      <el-table-column prop="buildingName" label="楼宇" width="180">
      </el-table-column>
      <el-table-column prop="dormitoryName" label="宿舍" width="180">
      </el-table-column>
      <el-table-column prop="studentName" label="学生" width="180">
      </el-table-column>
      <el-table-column prop="reason" label="原因" width="180">
      </el-table-column>
      <el-table-column prop="dormitoryAdminName" label="宿管" width="180">
      </el-table-column>
...
export default {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios
            .get("http://localhost:9000/absent/search", {
    ...
    page(currentPage) {
      ...
      if (_this.ruleForm.value == "") {
        axios
          .get(
            "http://localhost:9000/absent/list/" +
              currentPage +
              "/" +
              _this.pageSize
          )
  ...
  created() {
    const _this = this;
    axios
      .get("http://localhost:9000/absent/list/1/" + _this.pageSize)
      .then(function (resp) {
        _this.tableData = resp.data.data.data;
        _this.total = resp.data.data.total;
      });
