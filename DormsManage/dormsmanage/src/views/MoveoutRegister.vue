...
export default {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios
            .get("http://localhost:9000/moveout/search", {
    ...
    page(currentPage) {
      ...
      if (_this.ruleForm.value == "") {
        axios
          .get(
            "http://localhost:9000/moveout/list/" +
              currentPage +
              "/" +
              _this.pageSize
          )
    ...
    moveout(row) {
      ...
      this.$prompt("请输入迁出原因", "提示", {
        ...
        this.$confirm("确认迁出【" + row.name + "】吗？", "提示", {
          ...
          axios
            .put(
              "http://localhost:9000/moveout/moveout/" + row.id + "/" + value
            )
            .then(function (resp) {
              if (resp.data.code == 0) {
  ...
  created() {
    const _this = this;
    axios
      .get("http://localhost:9000/moveout/list/1/" + _this.pageSize)
      .then(function (resp) {
        _this.tableData = resp.data.data.data;
        _this.total = resp.data.data.total;
      });
